  ...
  return (
    <article
      className={`rounded-2xl border bg-white p-4 shadow-sm transition ${
        isExpanded ? "border-sky-300 ring-2 ring-sky-100" : "border-slate-200 hover:-translate-y-0.5 hover:shadow-soft"
      }`}
    >
      ...
      {!isExpanded ? (
        <p className="mt-4 text-sm text-slate-700">
          Atiende a {resource.poblacion.join(", ")}. {resource.barrio ? `Zona: ${resource.barrio}.` : null}
        </p>
      ) : null}
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <button
          type="button"
          onClick={onToggle}
          className="inline-flex min-h-11 items-center justify-center rounded-xl bg-sky-700 px-4 text-sm font-bold text-white transition hover:bg-sky-800 focus:outline-none focus:ring-4 focus:ring-sky-100"
